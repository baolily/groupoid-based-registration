...
class LDDMMDiscontinuousMapNet(ShootingVectorMomentumNet):
    """
    LDDMM with discontinuous EPDiff equation for vortex sheet dynamics.
    Uses FEniCS-based discontinuous smoother for velocity-momentum relationship.

    Implements the discontinuous EPDiff equation:
    ∂m^±/∂t + (i_v d^R m^± + 1/2 d^R i_v m^± + 1/2 div^R(v) · m^±) = 0

    Where the velocity-momentum relationship is: I(u) = (Id - Δ)^R u + I_Γ(u) = m
    """

    def __init__(self, sz, spacing, params, compute_inverse_map=False, mask_path=None):
        """
        Initialize discontinuous LDDMM network.

        Args:
            sz: image size
            spacing: spatial spacing
            params: parameter dictionary
            compute_inverse_map: whether to compute inverse map
            mask_path: path to segmentation mask for interface definition
        """
        super(LDDMMDiscontinuousMapNet, self).__init__(sz, spacing, params)

        self.compute_inverse_map = compute_inverse_map
        """If set to True the inverse map is computed on the fly"""

        self.mask_path = mask_path
        """Path to mask defining the sliding interface"""

        # Create FEniCS discontinuous smoother
        self._create_discontinuous_smoother()

        # Create parameters for interior and exterior momentum fields
        self.create_registration_parameters()

        # Initialize interior/exterior momentum placeholders
        self.m_interior = None
        """Interior momentum field (will be computed during forward pass)"""
        self.m_exterior = None
        """Exterior momentum field (will be computed during forward pass)"""

        self.integrator = self.create_integrator()
        """integrator to solve discontinuous EPDiff equation"""

    def _create_discontinuous_smoother(self):
        """Create FEniCS-based discontinuous smoother"""
        # Get smoother dict using standard ParameterDict tuple access.
        # Keep any user-provided values intact instead of reinitializing them.
        smoother_dict = self.params[('smoother', {}, 'settings for smoother')]

        smoother_dict['type'] = 'fenics_discontinuous'
        smoother_dict[('use_advanced_interface_terms', True, 'whether to use advanced interface terms')]
        smoother_dict[('fenics_solver_params', {}, 'parameters for the FEniCS discontinuous solver')]

        if self.mask_path is not None:
            smoother_dict['mask_path'] = self.mask_path
        else:
            smoother_dict[('mask_path', '', 'path to the interface mask')]

        try:
            configured_mask_path = smoother_dict['mask_path']
        except Exception:
            configured_mask_path = self.mask_path

        print(f"Creating discontinuous smoother with mask_path: {configured_mask_path}")

        self.discontinuous_smoother = SF.SmootherFactory(self.sz[2:], self.spacing).create_smoother(self.params)

    def create_registration_parameters(self):
        """
        Create registration parameters for interior and exterior momentum fields
        """
        # Create single combined momentum field using parent class method
        # We'll apply region decomposition during forward pass
        m_combined = utils.create_ND_vector_field_parameter_multiN(
            self.sz[2::], self.nrOfImages,
            self.get_momentum_from_external_network
        )
        return m_combined
    ...
    def create_integrator(self):
        """
        Creates an integrator for discontinuous EPDiff + advection equation
        """
        cparams = self.params[('forward_model', {}, 'settings for the forward model')]

        # Use our new discontinuous EPDiff forward model
        epdiffDiscontinuous = FM.EPDiffDiscontinuousMap(
            self.sz, self.spacing, self.discontinuous_smoother, cparams,
            compute_inverse_map=self.compute_inverse_map,
            mask_path=self.mask_path
        )

        return ODE.ODEWrapBlock(epdiffDiscontinuous, cparams, self.use_odeint, self.use_ode_tuple, self.tFrom, self.tTo)

    def forward(self, phi, I0_source, phi_inv=None, variables_from_optimizer=None):
        """
        Solves discontinuous EPDiff + advection equation forward and returns the map at time tTo
        ...
        :return: returns the map at time tTo
        """
        # Set source image for discontinuous smoother
        self.discontinuous_smoother.set_source_image(I0_source)

        # Prepare parameters to pass to integrator
        pars_to_pass = self._get_default_dictionary_to_pass_to_integrator()
        pars_to_pass['I0'] = I0_source

        self.integrator.init_solver(pars_to_pass, variables_from_optimizer, has_combined_input=True)

        # Use the combined momentum field and decompose into interior/exterior regions
        m_combined = self.m

        # Get mask from discontinuous smoother for region decomposition
        # The mask defines the sliding interface Γ
        if hasattr(self.discontinuous_smoother, 'mask') and self.discontinuous_smoother.mask is not None:
            mask = self.discontinuous_smoother.mask
        elif hasattr(self.integrator.model, 'mask') and self.integrator.model.mask is not None:
            mask = self.integrator.model.mask
        else:
            raise ValueError("Mask not found in discontinuous smoother or forward model")

        # Ensure mask has compatible spatial dimensions with momentum field
        if mask.shape[-2:] != m_combined.shape[-2:]:
            mask = torch.nn.functional.interpolate(mask, size=m_combined.shape[-2:], mode='nearest')

        # Decompose momentum: m = χ_Γ^+ m^+ + χ_Γ^- m^-
        self.m_interior = m_combined * mask
        self.m_exterior = m_combined * (1 - mask)

        # Pass decomposed momentum fields to integrator
        # State format: [m_interior, m_exterior, phi] or [m_interior, m_exterior, phi, phi_inv]
        if self.compute_inverse_map:
            if phi_inv is not None:
                result = self.integrator.solve([self.m_interior, self.m_exterior, phi, phi_inv], variables_from_optimizer)
                # Return final phi and phi_inv (result[2] and result[3])
                return (result[2], result[3])
            else:
                result = self.integrator.solve([self.m_interior, self.m_exterior, phi], variables_from_optimizer)
                # Return final phi and None (result[2])
                return (result[2], None)
        else:
            result = self.integrator.solve([self.m_interior, self.m_exterior, phi], variables_from_optimizer)
            # Return final phi (result[2])
            return result[2]

    def compute_regularization_energy(self, I0_source, variables_from_optimizer=None):
        """
        Compute regularization energy using DVect norm for discontinuous fields
        """
        # Combine interior and exterior momentum
        m_combined = self.m_interior + self.m_exterior

        # Prepare parameters
        pars_to_pass = self._get_default_dictionary_to_pass_to_integrator()
        pars_to_pass = utils.combine_dict(
            {'I': I0_source}, pars_to_pass
        )

        # Compute velocity from combined momentum using discontinuous smoother
        v = self.discontinuous_smoother.smooth(
            m_combined, None, pars_to_pass, variables_from_optimizer,
            smooth_to_compute_regularizer_energy=True
        )

        # Compute DVect inner product: ⟨I(m), v⟩ = ⟨m, v⟩
        # Convert spacing.prod() to torch tensor to avoid numpy/torch type mismatch
        spacing_prod = torch.tensor(self.spacing.prod(), dtype=torch.float32, device=m_combined.device)
        reg_energy = torch.sum(m_combined * v) * spacing_prod

        # Add penalty from discontinuous smoother (if any)
        reg_energy += self.discontinuous_smoother.get_penalty()

        return reg_energy

    def get_variables_to_transfer_to_loss_function(self):
        """
        Return discontinuous smoother for use in loss function
        """
        d = dict()
        d['smoother'] = self.discontinuous_smoother
        d['initial_velocity'] = None  # Not computed for discontinuous case yet
        return d

    def get_custom_optimizer_output_string(self):
        """
        Custom output string for optimizer showing momentum norms
        """
        if self.m_interior is not None and self.m_exterior is not None:
            return f", m_int_norm={torch.norm(self.m_interior).item():.3e}, " + \
                   f"m_ext_norm={torch.norm(self.m_exterior).item():.3e}" + \
                   self.discontinuous_smoother.get_custom_optimizer_output_string()
        else:
            return self.discontinuous_smoother.get_custom_optimizer_output_string()

    def get_custom_optimizer_output_values(self):
        """
        Custom output values for optimizer showing momentum statistics
        """
        values = {}

        if self.m_interior is not None:
            values['m_interior_norm'] = torch.norm(self.m_interior).detach().cpu().numpy()
        if self.m_exterior is not None:
            values['m_exterior_norm'] = torch.norm(self.m_exterior).detach().cpu().numpy()

        smoother_values = self.discontinuous_smoother.get_custom_optimizer_output_values()
        if smoother_values is not None:
            values.update(smoother_values)

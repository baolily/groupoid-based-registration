...
def create_ND_vector_field_parameter_multiN(sz, nrOfI=1,get_field_from_external_network=False):
    ...
    if get_field_from_external_network:
        # Use larger initialization (0.01) for better optimization starting point
        tmp = MyTensor(*(csz.tolist())).normal_(0.,0.01)
        tmp.requires_grad = True
    else:
        # Use larger initialization (0.01) for better optimization starting point
        tmp = Parameter(MyTensor(*(csz.tolist())).normal_(0.,0.01))
    return tmp

...
class Smoother(with_metaclass(ABCMeta, object)):
    ...
    def __init__(self, sz, spacing, params):
        self.sz = sz
        """image size"""
        # CRITICAL: Ensure spacing is a proper numpy array, not a 0-dim tensor/scalar
        # This prevents "invalid index of a 0-dim tensor" errors in _do_CFL_clamping_if_necessary
        if hasattr(spacing, 'detach'):
            spacing = spacing.detach().cpu().numpy()
        elif hasattr(spacing, 'cpu'):
        ...
        # Ensure volumeElement is a Python float (not 0-dim array)
        self.volumeElement = float(np.prod(self.spacing))
    ...
    def _do_CFL_clamping_if_necessary(self,v, clampCFL_dt):
        ...
        if clampCFL_dt is not None:
            ...
            for d in range(self.dim):
                # Extract spacing element as float to avoid 0-dim tensor indexing errors
                spacing_d = float(self.spacing[d]) if hasattr(self.spacing, '__getitem__') else float(self.spacing)
                if (torch.abs(v[:,d,...].data)).max()>=spacing_d/clampCFL_dt*rk4_factor:
                    need_to_clamp = True
                    break
            ...
            if need_to_clamp:
                ...
                for d in range(self.dim):
                    # Extract spacing element as float to avoid 0-dim tensor indexing errors
                    spacing_d = float(self.spacing[d]) if hasattr(self.spacing, '__getitem__') else float(self.spacing)
                    cmax = spacing_d/clampCFL_dt*rk4_factor
...
class FenicsDiscontinuousSmoother(Smoother):
    """
    FEniCS-based discontinuous smoother that solves I(u) = m
    where I(u) = (Id - Δ)^R u + I_Γ(u) with sliding boundary conditions.
    """

    @staticmethod
    def _read_optional_param(container, key, default):
    ...
    def __init__(self, sz, spacing, params):
        # CRITICAL: Convert spacing to numpy array BEFORE passing to parent class
        # This prevents "invalid index of a 0-dim tensor" errors
        # If spacing is a PyTorch tensor, spacing.prod() returns a 0-dim tensor
        # which causes indexing errors throughout the codebase
        if hasattr(spacing, 'detach'):
            spacing_np = spacing.detach().cpu().numpy()
        elif hasattr(spacing, 'cpu'):
            spacing_np = spacing.cpu().numpy()
        else:
            spacing_np = np.asarray(spacing)

        # Ensure spacing is a proper 1D array, not a 0-dim scalar
        spacing_np = np.atleast_1d(spacing_np)

        # Also convert sz to ensure it's a proper array/tuple
        if hasattr(sz, 'detach'):
            sz = sz.detach().cpu().numpy()
        elif hasattr(sz, 'cpu'):
            sz = sz.cpu().numpy()

        # Ensure sz is an integer array
        sz = np.asarray(sz, dtype=np.int64)

        super(FenicsDiscontinuousSmoother, self).__init__(sz, spacing_np, params)

        # FEniCS smoother specific parameters.
        # Support both regular dicts and the project's ParameterDict class.
        self.use_advanced_interface_terms = self._read_optional_param(
            params, 'use_advanced_interface_terms', True
        )
        self.mask_path = self._read_optional_param(params, 'mask_path', '')
        self.fenics_solver_params = self._read_optional_param(params, 'fenics_solver_params', {})

        # Initialize solver when mask is available
        self.velocity_solver = None
        self.mask_np = None

        print(f"FenicsDiscontinuousSmoother initialized (sz={sz})")
        print(f"  Advanced interface terms: {self.use_advanced_interface_terms}")
        print(f"  Mask path: {self.mask_path}")

        # Load mask if path is provided
        if self.mask_path and self.mask_path != '':
            self._load_mask_from_path()

    def _load_mask_from_path(self):
        """Load mask from file path and resize to match expected image size."""
        import skimage.io as skio
        import skimage.transform as skt

        try:
            print(f"Loading mask from: {self.mask_path}")
            mask_img = skio.imread(self.mask_path)

            # Convert to grayscale if needed
            if len(mask_img.shape) > 2:
                mask_img = mask_img[..., 0]

            # Get expected shape from self.sz
            # self.sz could be (H, W) for 2D or (H, W, D) for 3D
            # It does NOT include batch or channel dimensions
            expected_shape = tuple(self.sz)

            print(f"Expected image shape from sz: {expected_shape}")
            print(f"Loaded mask shape: {mask_img.shape}")

            # Resize if needed
            if mask_img.shape != expected_shape:
                print(f"Resizing mask from {mask_img.shape} to {expected_shape}")
                mask_img = skt.resize(mask_img, expected_shape,
            ...
            self.mask_np = mask_img.astype(np.float32)
            print(f"Mask loaded and resized to {self.mask_np.shape}")

            # Initialize FEniCS solver with the loaded mask
            self._initialize_solver()

        except Exception as e:
            print(f"Warning: Failed to load mask from {self.mask_path}: {e}")
            print("FEniCS solver will not be initialized")
            import traceback
            traceback.print_exc()

    def set_mask(self, mask_tensor):
        """
        Set the segmentation mask for discontinuous regions.

        Args:
            mask_tensor: PyTorch tensor [B, C, H, W] with region mask
        """
        if mask_tensor is not None:
            ...
            # Check if mask size matches expected image size (self.sz)
            # self.sz is (H, W) for 2D or (H, W, D) for 3D (no batch/channel dims)
            expected_shape = tuple(self.sz)

            print(f"set_mask: Expected shape {expected_shape}, got mask shape {mask_np.shape}")

            if mask_np.shape != expected_shape:
                print(f"Warning: Mask shape {mask_np.shape} does not match expected {expected_shape}")
                print(f"Resizing mask to match image dimensions...")

                # Resize mask using scipy or skimage
                import skimage.transform as skt
                self.mask_np = skt.resize(mask_np, expected_shape,
                                         order=0,  # Nearest neighbor for binary mask
                                         preserve_range=True,
                                         anti_aliasing=False)
                print(f"Mask resized to {self.mask_np.shape}")
            else:
    ...
    def _initialize_solver(self):
        """Initialize the FEniCS velocity solver with current mask."""
        if self.mask_np is not None:
            try:
                # Import the FEniCS velocity solver
                from . import fenics_velocity_solver as fvs

                # Create the FEniCS solver
                if fvs.FENICS_AVAILABLE:
                    print(f"Creating FEniCS velocity solver for shape {self.mask_np.shape}")

                    # self.spacing is already a numpy array due to conversion in __init__
                    # But double-check to be safe
                    spacing_np = np.asarray(self.spacing)

                    r_val = int(self._read_optional_param(self.fenics_solver_params, 'R', 2))
                    alpha_val = float(self._read_optional_param(self.fenics_solver_params, 'alpha', 0.05))
                    penalty_val = float(
                        self._read_optional_param(self.fenics_solver_params, 'penalty_parameter', 50.0)
                    )

                    print(
                        "Using FEniCS solver parameters: "
                        f"R={r_val}, alpha={alpha_val}, penalty_parameter={penalty_val}"
                    )

                    self.velocity_solver = fvs.create_fenics_solver(
                        shape=self.mask_np.shape,
                        spacing=spacing_np,
                        mask=self.mask_np,
                        advanced=self.use_advanced_interface_terms,
                        verbose=True,  
                        R=r_val,
                        alpha=alpha_val,
                        penalty_parameter=penalty_val
                    )
                    print(f"✓ FEniCS velocity solver initialized successfully")
                else:
                    print("Warning: FEniCS not available, using fallback method")
                    self.velocity_solver = None

            except Exception as e:
                print(f"Warning: FEniCS solver initialization failed: {e}")
                print("Falling back to finite difference method")
                import traceback
                traceback.print_exc()
                self.velocity_solver = None

    def apply_smooth(self, v, vout=None, pars=dict(), variables_from_optimizer=None,
                     smooth_to_compute_regularizer_energy=False, clampCFL_dt=None):
        """
        Implementation of abstract method from base class.
        Smooth momentum m to get velocity v by solving I(u) = m.

        Args:
            v: momentum field [B, D, H, W] (renamed from m to match base class signature)
            vout: output velocity field (if provided)
            pars: dictionary containing additional parameters (including 'mask' if available)
            variables_from_optimizer: optimizer variables
            smooth_to_compute_regularizer_energy: flag for regularizer computation
            clampCFL_dt: CFL condition parameter (not used)

        Returns:
            velocity: velocity field [B, D, H, W]
        """
        if v is None:
            return None

        # Check if mask is provided in parameters
        if 'mask' in pars and self.mask_np is None:
            self.set_mask(pars['mask'])

        # If no FEniCS solver available, fall back to simple approximation
        if self.velocity_solver is None:
            velocity = self._fallback_smooth(v, vout)
        else:
            try:
                # Use FEniCS solver to compute velocity from momentum
                velocity = self.velocity_solver.solve_velocity_from_momentum(v)

                if vout is not None:
                    vout.data.copy_(velocity.data)
                    velocity = vout

            except Exception as e:
                print(f"\n{'='*60}")
                print(f"ERROR in FEniCS solve_velocity_from_momentum:")
                ...
                for line in tb_lines:
                    print(line, end='')
                print(f"{'='*60}\n")
                print("Using fallback finite difference method")
                velocity = self._fallback_smooth(v, vout)

        # Apply CFL clamping if needed
        try:
            velocity = self._do_CFL_clamping_if_necessary(velocity, clampCFL_dt=clampCFL_dt)
        except Exception as e:
            print(f"\n{'='*60}")
            print(f"ERROR in _do_CFL_clamping_if_necessary:")
            print(f"  Error type: {type(e).__name__}")
            print(f"  Error message: {e}")
            print(f"  self.spacing = {self.spacing}")
            print(f"  type(self.spacing) = {type(self.spacing)}")
            if hasattr(self.spacing, 'shape'):
                print(f"  self.spacing.shape = {self.spacing.shape}")
            print(f"  self.dim = {self.dim}")
            print(f"  clampCFL_dt = {clampCFL_dt}")
            print(f"{'='*60}")
            import traceback
            import sys
            exc_type, exc_value, exc_traceback = sys.exc_info()
    ...
    def _fallback_smooth(self, m, vout=None):
        """
        Fallback smoothing method using finite differences when FEniCS fails.
        """
        # Simple iterative solver for (Id - Δ)u ≈ m
        if isinstance(self.spacing, (list, tuple, np.ndarray)):
            spacing_min = min(self.spacing[0], self.spacing[1])
        else:
            spacing_min = self.spacing
            
        omega = 0.5 / (1.0 + 4.0 / (spacing_min ** 2))  
        
        velocity = m.clone()
        for iteration in range(20): 
            laplacian = self._compute_laplacian_fd(velocity)
            velocity = (1.0 - omega) * velocity + omega * (m + laplacian)

            # Apply mask-based region splitting if available
            if self.mask_np is not None:
                velocity = self._apply_mask_splitting(velocity)
    ...
    def _compute_laplacian_fd(self, field):
        """Compute Laplacian using finite differences."""
        # FD_torch has 'lap' method, not 'laplacian'
        # The lap method computes the Laplacian for each channel separately
        if len(field.shape) == 4:  # [B, C, H, W]
            B, C, H, W = field.shape
            result = torch.zeros_like(field)
            for c in range(C):
                result[:, c, :, :] = self.fdt.lap(field[:, c, :, :])
            return result
        else:
            # For other shapes, directly use lap
            return self.fdt.lap(field)

    def _apply_mask_splitting(self, velocity):
        """Apply mask-based region splitting to velocity field."""
        if self.mask_np is None:
            return velocity

        B, D, H, W = velocity.shape
        mask_tensor = AdaptVal(torch.from_numpy(self.mask_np[None, None, :, :]))

        # Resize mask to match velocity dimensions if needed
        ...
        mask_expanded = mask_tensor.expand(B, D, H, W)

        # Apply region-based processing (simplified)
        velocity_interior = velocity * mask_expanded
        velocity_exterior = velocity * (1 - mask_expanded)

        return velocity_interior + velocity_exterior

    def get_penalty(self):
        """
        Return penalty term from the FEniCS solver (if applicable).
        """
        # Return a proper torch tensor (not 0-dim) to avoid indexing issues
        # Using .item() ensures we have a Python float that can be safely converted
        if self.velocity_solver is not None and hasattr(self.velocity_solver, 'get_penalty'):
            return self.velocity_solver.get_penalty()
        else:
            # Return Python float 0, which PyTorch will handle correctly
            return 0.0

    def compute_regularizer_multiN(self, v):
        """
        Compute regularizer energy for discontinuous velocity field.
        This implements the DVect norm: ⟨I(u), v⟩
        """
        if self.velocity_solver is not None and hasattr(self.velocity_solver, 'compute_dvect_inner_product'):
            result = self.velocity_solver.compute_dvect_inner_product(v, v)
        else:
            # Fallback to L2 norm
            # Ensure volumeElement is a scalar (not a 0-dim array)
            vol_element = float(self.volumeElement)
            result = torch.sum(v**2) * vol_element

        # Return as a scalar tensor to avoid 0-dim indexing issues
        return result if isinstance(result, (int, float)) else result

    def compute_dvect_inner_product(self, u, v):
        """
        Compute DVect inner product: ⟨u, v⟩_{DVect(M,Γ)} := ⟨I(u), v⟩

        Args:
            u, v: velocity fields [B, 2, H, W]

        Returns:
            inner_product: scalar representing the DVect inner product
        """
        if self.velocity_solver is not None and hasattr(self.velocity_solver, 'compute_dvect_inner_product'):
            result = self.velocity_solver.compute_dvect_inner_product(u, v)
        else:
            # Fallback to L2 inner product
            # Ensure volumeElement is a scalar (not a 0-dim array)
            vol_element = float(self.volumeElement)
            result = torch.sum(u * v) * vol_element
...
class AvailableSmoothers(object):
    ...
    def __init__(self):
        ...
        self.smoothers = {
            ...
            'fenics_discontinuous': (FenicsDiscontinuousSmoother, 'FEniCS-based discontinuous velocity solver for I(u) = (Id - Δ)^R u + I_Γ(u) = m'),
        }
